...
The `max_cpus` / `max_memory` ceilings default to the starting `cpus`/`memory`, so reserve headroom at create time if the sandbox may need to grow live.

A live resize may not settle at once; the apply output reports a per-resource `STATE` of `applied`, `converging`, `failed`, or `guest-refused` (the host enforces the limit even when the guest declines). Add `--format json` for the same in `resize_status`.

`--secret NAME@HOST` adds or rotates a secret from the host environment variable of that name. Inline `NAME=VALUE@HOST` is rejected since it would leak into shell history; pass raw values through an SDK.
